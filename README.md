...
- **Big-number / KPI blocks**: a no-code metric — give an expression
  (`df['rev'].sum()`) and a label, and it renders one big styled number (with an
  optional Python number format like `,.2f`). Compiled server-side (`/api/kpi`):
  the expression is user Python (like a code cell), the label is HTML-escaped at
  runtime. Its reads are extracted from the expression by the same `ast` analyzer,
  so a KPI updates reactively when its inputs change.
- **Reactive execution** (opt-in **⚡ Reactive** toggle): when a block changes,
  ...
  mistaken for dependencies; a KPI's reads come from analyzing its expression),
  while SQL/input/chart blocks declare their reads and writes. A block B is
  downstream of A when B reads a name A writes, computed
...
web/                   Vite + React + TypeScript frontend
  src/
    ...
    lib/store.ts       zustand: block model (code/md/sql/input/chart/kpi), autosave rev
